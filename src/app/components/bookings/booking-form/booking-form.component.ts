...
export class BookingFormComponent implements OnInit, OnDestroy {
  ...
  public availableRooms: any[] | undefined;
  public selectedRooms: any[] | undefined;
  ...
  constructor(
    ...
    _store.select('rooms').subscribe((response: any) => {
      this.rooms$ = response;
    });

    _store.select('guests').subscribe((response: any) => {
    ...
    this._room.getRooms('availables').subscribe(
      (data: any) => {
        this.availableRooms = _.map(
          data,
          (item: { uid: any; name: any; type: any; max_guests: any }) => {
            return {
              id: item.uid,
      ...
      if (!isNaN(this.id)) {
        ...
        this._bookings.getBookingById(this.id).subscribe(
          (data: any) => {
            ...
            this.selectedRooms = _.map(
              data.room,
              (room: { uid: any; name: any; type: any; max_guests: any }) => {
                return {
                  id: room.uid,
