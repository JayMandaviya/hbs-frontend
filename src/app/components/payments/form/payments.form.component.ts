...
export class PaymentsFormComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private _payment: PaymentsController,
  ...
  public ngOnInit() {
    this.sub = this.route.params.subscribe((params) => {
      ...
      if (!isNaN(this.id)) {
        ...
        this._payment.getPaymentById(this.id).subscribe(
          (data: any) => {
            this.form = {
              amount: data.amount,
  ...
  public onSubmit(e: MouseEvent) {
    ...
    if (isNaN(this.id)) {
      ...
      // request create room
      this._payment.createPayment(this.form).subscribe(
        (data: any) => {
          this._store.dispatch({
      ...
      // request create room
      this._payment.updatePayment(this.id, this.form).subscribe(
        (data: any) => {
          this._store.dispatch({
